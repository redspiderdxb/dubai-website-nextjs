...
import { getEmailTemplateProps } from "./emailAssets";
...
function buildTeamPlainText({ name, email, phone, subject, content }) {
  return [
    "New website enquiry",
    "",
    `Name: ${name}`,
...
export async function buildTeamNotificationEmail(data) {
  const html = await render(TeamNotificationEmail(getEmailTemplateProps(data)));
...
export async function buildThankYouEmail(data) {
  const html = await render(ThankYouEmail(getEmailTemplateProps(data)));
...
export async function sendContactEmails({ name, email, phone, subject, content }) {
  ...
  const thankYouEmail = await buildThankYouEmail({ name });
