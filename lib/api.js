 ...
 * Fetch ALL products
 *
 * Uses the internal Next.js API proxy so the API key
  ...
  try {
    const proxyUrl =
      typeof window === "undefined"
        ? `http://localhost:${process.env.PORT || 3000}/api/products/`
        : "/api/products/";
    ...
    if (!res.ok) {
      throw new Error(`Products proxy API error: ${res.status}`);
    }
...
export const fetchProductBySlug = async (slug) => {
  try {
    ...
    const proxyUrl =
      typeof window === "undefined"
        ? `http://localhost:${process.env.PORT || 3000}/api/products/${encodeURIComponent(slug)}/`
    ...
    if (!res.ok) {
      throw new Error(`Product proxy API error: ${res.status}`);
    }
...
export const fetchHomepageData = async () => {
  try {
    ...
    const homepageData = response?.data || null;

    console.log("========== HOMEPAGE DATA ==========");
    console.log(homepageData);
    console.log("FAQ DATA:", homepageData?.faqs);
    console.log("FAQ COUNT:", homepageData?.faqs?.length);
    console.log("===================================");
