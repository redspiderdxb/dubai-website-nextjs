            ...
            <div className="col-lg-3">
              <div className="rs-gd-intro__rail">
                ...
                <span className="rs-gd-intro__rail-text">
                  {data.intro_small_heading || "Graphic Design · Dubai"}
                </span>
              </div>
            </div>
      ...
      <div className="premium-col premium-col-left">
        <div className="premium-services">
          ...
          <h2 className="premium-title">
            Our <span className="premium-highlight">Graphic Design</span> Services
          </h2>
          <p className="premium-subtitle">
            We offer a wide range of graphic design services tailored to your needs.
          </p>

          <div className="premium-accordion">
            {graphicServices.map((service, index) => (
              <div key={service.id} className="premium-accordion-item">
                <button 
                  className={`premium-accordion-btn ${index === 0 ? 'active' : ''}`}
                  onClick={(e) => {
                    const btn = e.currentTarget;
                    const item = btn.parentElement;
                    const body = item.querySelector('.premium-accordion-body');
                    const isActive = btn.classList.contains('active');
                    
                    // Close all accordion items in left column
                    const allItems = document.querySelectorAll('.premium-col-left .premium-accordion-item');
                    allItems.forEach(el => {
                      el.querySelector('.premium-accordion-btn').classList.remove('active');
                      el.querySelector('.premium-accordion-body').classList.remove('active');
                    });
                    ...
                    if (!isActive) {
                      ...
                      const images = document.querySelectorAll('.premium-main-image');
                      images.forEach(img => img.classList.remove('active'));
                      const targetImage = document.querySelector(`.premium-main-image-${index + 1}`);
                ...
                >
                  ...
                  <span className="premium-accordion-title">{service.title}</span>
                </button>
                <div className={`premium-accordion-body ${index === 0 ? 'active' : ''}`}>
                  <p className="premium-accordion-text">{service.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="premium-connect">
            <span className="premium-connect-label">Know More About:</span>
            <div className="premium-connect-line"></div>
            <a href="#" className="premium-connect-link">Contact us</a>
          </div>
        </div>
      </div>
      ...
      <div className="premium-col premium-col-right">
        <div className="premium-process">
          ...
          <p className="premium-process-subtitle">
            We work hard to create a graphic design that can keep our clients satisfied. Here is how it begins:
          </p>

          <div className="premium-process-accordion">
            {data.processes?.map((process, index) => (
              <div key={process.id || index} className="premium-process-item">
                <button 
                  className="premium-process-btn"
                  onClick={(e) => {
                    const btn = e.currentTarget;
                    const item = btn.parentElement;
                    const body = item.querySelector('.premium-process-body');
                    const isActive = item.classList.contains('active');
                    
                    // Close all process items in right column
                    const allItems = document.querySelectorAll('.premium-col-right .premium-process-item');
                    allItems.forEach(el => {
                ...
                >
                  <span className="premium-process-number">{index + 1}.</span>
                  <span className="premium-process-name">{process.title}</span>
                  <span className="premium-process-arrow">↗</span>
                </button>
                <div className="premium-process-body">
                  <p className="premium-process-text">{process.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container">
          ...
          <div className="row rs-agency-bottom align-items-center">
            <div className="letconnect mt-5">
              ...
              <a href={data.cta_button_link || "#"}>
                {data.cta_button_text || "Book A Call"}
              </a>
            </div>
          </div>
        </div>
